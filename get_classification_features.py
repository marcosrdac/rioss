...
augmentation_angles = [0,15,-30,60,-75]
n_augmentation = len(augmentation_angles)
...
#    plt.show()
    ...
    categories = discarray(labels_filename, 'w+', np.int64, ncoords*n_augmentation)
    cur = 0
    for C, c in enumerate(CLASSIFICATION_CATEGORIES):
        if c['has_coords']:
            n = c['coords'].shape[0] * n_augmentation
    ...
    for feature in CLASSIFICATION_FEATURES:
        ...
        feature['values'] = discarray(
            feature_filename, 'w+', np.float64, ncoords*n_augmentation)
...
#    # THIS PART IS YET NOT AUGMENTED
